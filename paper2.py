...
def scrape_new_papers(category_name, new_papers_url, file):
        ...
        for paper in papers:
            ...
            if title_tag:
                ...
                if author_tag:
                    ...
                    file.write(f"Match (C{iterator} {{title:\"{category_name}\"}})\n")
                    file.write(f"Merge (P{iterator})-[:isPaperIn]->(C{iterator})\n")

                    # Zapisz każdego autora w osobnym wierszu
                    author_iterator = 0
                    for author in authors:
                        author_iterator += 1
                        file.write(f"Merge (A{iterator}{author_iterator}:Author {{name:\"{author}\"}})\n")
                        file.write(f"Merge (P{iterator})-[:isWrittenBy]->(A{iterator}{author_iterator})\n")
            ...
            if ul_tag:
                ...
                for li_tag in ul_tag.find_all("li"):
                    ...
                    if main_link:
                        ...
                        scrape_new_papers(main_link_text, new_papers_url, file)
else:
